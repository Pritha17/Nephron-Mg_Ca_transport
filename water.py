...
def compute_water_fluxes (cell):
    ...
    if cell.segment == 'IMCD':
        '''
        Effect of CaSR on apical water permeability in IMCD
        Based on Edwards paper:
        Calcium reabsorption in the distal tubule: regulation by sodium, pH, and flow (2013)
        '''
        # CaSR effect on apical water permeability:
        base_apical_perm = cell.dLPV[0,1]
        alpha_imcd = -3/8
        EC_50_Ca = 1.25
        EC_50_Mg = 2.5
        cell.dLPV[0,1] = base_apical_perm*(
            1+alpha_imcd*(cell.conc[15,0]**4)/(cell.conc[15,0]**4+EC_50_Ca**4))*(
            1+0.6*alpha_imcd*(cell.conc[16,0]**4)/(cell.conc[16,0]**4+EC_50_Mg**4))

        cell.dLPV[1,0] = cell.dLPV[0,1]
