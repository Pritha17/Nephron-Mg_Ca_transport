...
def Jac(func, x, k):
    ...
    for i in range(len(x)):
        ...
        for j in range(len(x)):
            ...
            #if i == 33 and j==49:
             #   print(x[i]+h,j,i,wa1[j], fvec[j], h, abs(temp), (-wa1[j] + fvec[j]), (-wa1[j] + fvec[j]) / h)
    ...
    while (np.linalg.norm(f) > 0.0001) and (iter < maxiter + 1):  # (iter<300)
        if np.linalg.norm(f) > 1e14 or np.isnan(np.linalg.norm(f)):
            if cell.segment == 'CCD' or cell.segment == 'OMCD' or cell.segment == 'IMCD':
                ...
                raise Exception(
                    'Newton solver diverged in ' + cell.type + ' ' + cell.segment + ' at cell number: ' + str(k))
        elif iter == maxiter:
            if cell.segment == 'CCD' or cell.segment == 'OMCD' or cell.segment == 'IMCD':
                print('Warning!!: Newton solver did not converge in <' + str(
                    iter) + ' iterations in ' + cell.segment + ' cell number ' + str(k) + '\n')
            else:
                print('Warning!!: Newton solver did not converge in <' + str(
                    iter) + ' iterations in ' + cell.type + ' ' + cell.segment + ' cell number ' + str(k) + '\n')
        ...
        #print(F.shape)
        #for p in range(58):
         #   if F[0,p]*IJ.T[p,33] < -40:
          #      print(p)
            #print(iter, F[0,p]*IJ.T[p,33])
        #print(F[0,49], IJ.T[49,33], delta[33])
        #print(np.sum(IJ.T[:,32]))
        x -= delta
        f = np.matrix(fun(x, k))
        iter += 1
        # print(iter, np.linalg.norm(f))
