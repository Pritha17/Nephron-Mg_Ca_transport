...
def output_segment_results(cell,sup_or_jux,Scaletorq,file_to_save,N):
    ...
    for i in range(NS):
        ...
        file.close()
    for i in range(NS):
        file=open('./'+file_to_save+'/'+sex_or_preg+'_'+cell[0].species+'_'+cell[0].segment+'_con_of_'+solute[i]+'_in_LIS'+sup_or_jux+'.txt','w')
        for j in range(N):
            file.write(str(cell[j].conc[i,4])+'\n')
        file.close()
    for i in range(NS):
    ...
    for j in range(N):
        ...
        jvol,jsol, electro_flux,convective_flux, active_jsol  = flux.compute_fluxes(cell[j],j)            
        ...
        file_Ca_para = open('./' + file_to_save + '/' + sex_or_preg + '_' + cell[0].species + '_' + cell[j].segment + '_paracellular_Ca' + sup_or_jux + '.txt', 'a')
        file_Ca_para.write(str(jsol[15, 0, 4]) + '\n')

        file_Mg_para = open('./' + file_to_save + '/' + sex_or_preg + '_' + cell[0].species + '_' + cell[j].segment + '_paracellular_Mg' + sup_or_jux + '.txt', 'a')
        file_Mg_para.write(str(jsol[16, 0, 4]) + '\n')
        ...
        if cell[j].segment!='SDL' and cell[j].segment!='LDL' and cell[j].segment!='LAL':
            for i in range(len(cell[j].trans)):
                ...
                if transporter_type == 'SGLT1':
                    ...
                    if cell[0].segment == 'PT' or cell[0].segment == 'S3':
                        solute_id,fluxs=ATPase.calatapse(cell[j],cell[j].ep,memb_id,cell[j].trans[i].act,cell[j].area)
                    if cell[0].segment == 'mTAL' or cell[0].segment == 'cTAL' or cell[0].segment =='CNT' or cell[0].segment == 'DCT':
                        solute_id,fluxs=ATPase.calatapse_mtal(cell[j],j,cell[j].ep,memb_id,cell[j].trans[i].act,cell[j].area)
                    for k in range(len(solute_id)):
                        file = open('./'+file_to_save+'/'+sex_or_preg+'_'+cell[0].species+'_'+cell[j].segment+'_'+transporter_type+'_'+solute[solute_id[k]]+sup_or_jux+'.txt','a')
                        file.write(str(fluxs[k]*Scaletorq[j])+'\n')
                    ...
                    solute_id,fluxs=NCX1.ncx1(cell[j],j,cell[j].ep,memb_id,cell[j].trans[i].act,cell[j].area)
                    for k in range(len(solute_id)):
                        file = open('./'+file_to_save+'/'+sex_or_preg+'_'+cell[0].species+'_'+cell[j].segment+'_'+transporter_type+'_'+solute[solute_id[k]]+sup_or_jux+'.txt','a')
                        file.write(str(fluxs[k]*Scaletorq[j])+'\n')
                elif transporter_type == 'TRPV5':
                    solute_id,fluxs=TRPV5.trpv5(cell[j],j,cell[j].ep,memb_id,cell[j].trans[i].act,cell[j].area)
                    for k in range(len(solute_id)):
                        file = open('./'+file_to_save+'/'+sex_or_preg+'_'+cell[0].species+'_'+cell[j].segment+'_'+transporter_type+'_'+solute[solute_id[k]]+sup_or_jux+'.txt','a')
                        file.write(str(fluxs[k]*Scaletorq[j])+'\n')
                elif transporter_type == 'TRPM6':
                    solute_id,fluxs=TRPM6.trpm6(cell[j],cell[j].ep,memb_id,cell[j].trans[i].act,cell[j].area)
                    for k in range(len(solute_id)):
                        file = open('./'+file_to_save+'/'+sex_or_preg+'_'+cell[0].species+'_'+cell[j].segment+'_'+transporter_type+'_'+solute[solute_id[k]]+sup_or_jux+'.txt','a')
                        file.write(str(fluxs[k]*Scaletorq[j])+'\n')
                elif transporter_type == 'NaMgX':
                    solute_id,fluxs=NaMgX.namgx(cell[j],memb_id,cell[j].trans[i].act,cell[j].area)
                    for k in range(len(solute_id)):
                        file = open('./'+file_to_save+'/'+sex_or_preg+'_'+cell[0].species+'_'+cell[j].segment+'_'+transporter_type+'_'+solute[solute_id[k]]+sup_or_jux+'.txt','a')
                        file.write(str(fluxs[k]*Scaletorq[j])+'\n')
                else:
