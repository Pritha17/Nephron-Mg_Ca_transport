...
# hNaMP is the activity of ENaC from .dat file
    ...
    ph = np.zeros(NC)
    for j in [0,1]:
        ph[j] = -np.log(abs(cell.conc[11][j])/1e3)/np.log(10.0)
    ...
    if cell.segment =='DCT':
        ...
        facphMP =  (1/(0.5+np.exp(-2.6*(ph[1]-7.20))))
        ...
        '''
        Ca2+:
        Effect of Ca2+ on ENaC
        Based on Edwards paper:
        Calcium reabsorption in the distal tubule: regulation by sodium, pH, and flow (2013)
        '''
        if cell.conc[15,1]> 150e-6:
            f_cai = np.sqrt(150e-6/cell.conc[15,1])
        else:
            f_cai = 1

        hENaC=hNaMP*facNaMP*facCaMP*facphMP*facFvMP*f_cai
