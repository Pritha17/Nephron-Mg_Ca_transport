...
def hatpase(cell,ep,memb_id,act,area):
    ...
    if memb_id[0]==0 and memb_id[1]==3 and cell.segment =='OMCD':
        # CaSR effect on OMCD 
            alpha_omcd = +2
            EC_50_Ca = 1.25
            EC_50_Mg = 2.5
            fluxHATPase = fluxHATPase*(
                1+alpha_omcd*(cell.conc[15,1]**4)/(cell.conc[15,1]**4+EC_50_Ca**4))* (
            1 + 0.6 * alpha_imcd * (cell.conc[16, 1] ** 4) / (cell.conc[16, 1] ** 4 + EC_50_Mg ** 4))
...
def calatapse(cell,ep,memb_id,act,area):

    AffCa = 75.6e-6 #from paper
